...
{
	int width = wdw->rect.w/6;
	int height = wdw->rect.h/10;
	int x = x1-width/2;
	int y = y1-height/2;
	...
	if (button->normalimg.magic != KISS_MAGIC)
	{
		...
		button->normalimg.w = width;
		button->normalimg.h = height;
	}
	if (button->activeimg.magic != KISS_MAGIC)
	{
		button->activeimg = kiss_active;
		button->activeimg.w = width;
		button->activeimg.h = height;
	}
	if (button->prelightimg.magic != KISS_MAGIC)
	{
		button->prelightimg = kiss_prelight;
		button->prelightimg.w = width;
		button->prelightimg.h = height;
	}
	kiss_makerect(&button->rect, x, y, width,
		height);

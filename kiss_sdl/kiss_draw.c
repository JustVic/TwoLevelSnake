...
void kiss_setup_fontsize(int textfont_size, int buttonfont_size)
{
	kiss_textfont_size = textfont_size;
	kiss_buttonfont_size = buttonfont_size;
}
...
SDL_Renderer* kiss_init(char* title, kiss_array *a, int w, int h)
{
	...
	window = SDL_CreateWindow(title, srect.w / 2 - w / 2,
		srect.h / 2 - h / 2, w, h, SDL_WINDOW_SHOWN|SDL_WINDOW_FULLSCREEN);
	if (window) kiss_array_append(a, WINDOW_TYPE, window);
	renderer = SDL_CreateRenderer(window, -1,
		SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
	if (renderer) kiss_array_append(a, RENDERER_TYPE, renderer);
	r += kiss_font_new(&kiss_textfont, "./data/kiss_font.ttf", a,
		kiss_textfont_size);
	r += kiss_font_new(&kiss_buttonfont, "./data/kiss_font.ttf", a,
		kiss_buttonfont_size);
	r += kiss_image_new(&kiss_normal, "./data/kiss_normal.png", a, renderer);
	r += kiss_image_new(&kiss_prelight, "./data/kiss_prelight.png", a, renderer);
	r += kiss_image_new(&kiss_active, "./data/kiss_active.png", a, renderer);
	r += kiss_image_new(&kiss_bar, "./data/kiss_bar.png", a, renderer);
	r += kiss_image_new(&kiss_vslider, "./data/kiss_vslider.png", a, renderer);
	r += kiss_image_new(&kiss_hslider, "./data/kiss_hslider.png", a, renderer);
	r += kiss_image_new(&kiss_up, "./data/kiss_up.png", a, renderer);
	r += kiss_image_new(&kiss_down, "./data/kiss_down.png", a, renderer);
	r += kiss_image_new(&kiss_left, "./data/kiss_left.png", a, renderer);
	r += kiss_image_new(&kiss_right, "./data/kiss_right.png", a, renderer);
	r += kiss_image_new(&kiss_combo, "./data/kiss_combo.png", a, renderer);
	r += kiss_image_new(&kiss_selected, "./data/kiss_selected.png", a, renderer);
	r += kiss_image_new(&kiss_unselected, "./data/kiss_unselected.png", a,
		renderer);
	if (r) {
		kiss_clean(a);
		return NULL;
